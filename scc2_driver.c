		   ...
		   uint32_t *IV, scc_cypher_mode_t cypher_mode)
{
	...
	if (status == SCC_RET_OK) {
		...
		/* Only write the IV if it will actually be used. In CBC mode
		 * the IV is internally updated with the previous block ciphertext
		 * therefore NULL can be passed when ciphering blocks > 1st.
		 */
		if (cypher_mode == SCC_CYPHER_MODE_CBC && IV != NULL) {
